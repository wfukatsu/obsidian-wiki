...
var LLMWikiSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Anthropic API Key").setDesc("Claude API key for LLM operations").addText(
      (text) => text.setPlaceholder("sk-ant-...").setValue(this.plugin.settings.apiKey).onChange(async (value) => {
        this.plugin.settings.apiKey = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Model").setDesc("Claude model to use").addDropdown(
      (dropdown) => dropdown.addOption("claude-sonnet-4-20250514", "Claude Sonnet 4").addOption("claude-opus-4-20250514", "Claude Opus 4").addOption("claude-haiku-4-5-20251001", "Claude Haiku 4.5").setValue(this.plugin.settings.model).onChange(async (value) => {
        this.plugin.settings.model = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Wiki directory").setDesc("Directory for LLM-generated wiki pages").addText(
    ...
    new import_obsidian.Setting(containerEl).setName("Sources directory").setDesc("Directory for raw source documents").addText(
    ...
    new import_obsidian.Setting(containerEl).setName("Max tokens").setDesc("Maximum tokens per LLM response").addText(
...
var LLMClient = class {
  constructor(settings) {
    this.client = new sdk_default({ apiKey: settings.apiKey });
...
Please audit this wiki for issues.`;
  ...
  return {
    ...
    healthScore: (_a2 = parsed.healthScore) != null ? _a2 : 0,
    summary: parsed.summary || ""
  };
}
